...
class FormEditAddress extends StatefulWidget {
  final UserDetailModel addressToEdit;
  ...
  @override
  _FormEditAddressState createState() => _FormEditAddressState();
}

class _FormEditAddressState extends State<FormEditAddress> {
  ...
  getInit() async {
    Future.wait([
      ...
      placesProvider.getDistrictsByRegency(widget.addressToEdit.regency),
    ]);
    userDetailProvider.initDefaultValInEdit(widget.addressToEdit.defaultAddress);
    _selectedRegency = widget.addressToEdit.regency;
    _selectedDistrict = widget.addressToEdit.district;

    ownerNameController = TextEditingController(text: widget.addressToEdit.addressOwner);
    phoneNumberController = TextEditingController(text: widget.addressToEdit.phoneNumber);
    addressController = TextEditingController(text: widget.addressToEdit.address);

    if(widget.addressToEdit.addressNotes != null) {
      detailController = TextEditingController(text: widget.addressToEdit.addressNotes);
    } else {
      detailController = TextEditingController(text: '');
    }

    mapIsCreated = false;
    latitudeForMap = widget.addressToEdit.latitude;
    longitudeForMap = widget.addressToEdit.longitude;
  }
  ...
  Widget build(BuildContext context) {
    ...
    handleEditAddress() async{
      if(await userDetailProvider.updateAddress(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: const Text('Data alamat berhasil diubah'), backgroundColor: secondaryColor, duration: const Duration(seconds: 2),),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: const Text('Data gagal diubah'), backgroundColor: alertColor, duration: const Duration(seconds: 2),),
        );
      }
    }
    ...
    return Scaffold(
        appBar: customAppBar(text: 'Form Ubah Alamat'),
